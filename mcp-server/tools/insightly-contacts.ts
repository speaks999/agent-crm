...
import { InsightlyApiError, InsightlyClient, InsightlyContact, InsightlyContactPayload } from '../utils/insightly-client.js';
...
const updateInsightlyContactSchema = createInsightlyContactSchema.partial().extend({
    ...
    const keys = Object.keys(value).filter((key) => key !== 'contact_id' && value[key as keyof typeof value] !== undefined);
    if (keys.length === 0) {
        ctx.addIssue({
...
export async function handleInsightlyContactTool(request: any, client: InsightlyClient) {
    ...
    try {
        switch (toolName) {
            ...
            case 'insightly_create_contact': {
                ...
                return respondWithContact(
                    `Created Insightly contact "${formatContactName(contact)}" (ID ${contact.CONTACT_ID}).`,
                    contact
                );
            }
            case 'insightly_update_contact': {
                ...
                const contact = await client.updateContact(payload as InsightlyContactPayload & { CONTACT_ID: number });
                return respondWithContact(
                    `Updated Insightly contact "${formatContactName(contact)}" (ID ${contact.CONTACT_ID}).`,
                    contact
                );
            }
...
function buildContactPayload(input: Partial<z.infer<typeof createInsightlyContactSchema>>): InsightlyContactPayload {
    const payload: InsightlyContactPayload = {};

    const directMap: Record<string, keyof InsightlyContactPayload> = {
    ...
    Object.entries(directMap).forEach(([inputKey, insightlyKey]) => {
        ...
        if (value !== undefined) {
            payload[insightlyKey] = value as never;
        }
    });
...
function respondWithContact(message: string, contact: InsightlyContact) {
    return {
        content: [
            {
                type: 'text',
...
function formatContactName(contact: InsightlyContact) {
